      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white py-8 px-4 shadow sm:rounded-xl sm:px-10 border-t-4 border-medical-dark">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div><label className="block text-sm font-medium text-gray-700">Doctor UPAHAAR ID or Email</label><input required className="mt-1 w-full border border-gray-300 rounded-lg p-3" type="text" placeholder="UPHR-XXXXXX or you@email.com" value={upahaarId} onChange={(e) => setUpahaarId(e.target.value)} /></div>
            <div><label className="block text-sm font-medium text-gray-700">Password</label><input required className="mt-1 w-full border border-gray-300 rounded-lg p-3" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} /><div className="text-right mt-1"><a href="/auth/doctor/forgot-password" className="text-xs font-semibold text-medical-dark hover:underline">Forgot Password?</a></div></div>
